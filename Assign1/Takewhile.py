...
def efficiency(item):
    eff =  sorted(items, key=lambda item:item[2]/item[1], reverse=True)
    return eff



def pack_bag(eff):
    name, weight, value = eff
    pack_bag.max_weight -= weight
    return pack_bag.max_weight >= 0    

pack_bag.max_weight = cap   

# pack the most efficient item until pack_bag.max_weight is reached.
pack = list(takewhile(pack_bag, reversed(sorted(items, key=efficiency))))

#pack = list(filter(pack_bag, sorted(items, key=item_efficiency, reverse=True)))
